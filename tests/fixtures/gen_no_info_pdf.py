#!/usr/bin/env python3
"""Generate a single-page PDF whose trailer has no /Info entry at all.

No third-party libraries. Emits a PDF 1.7 file with:
  * catalog -> pages -> one page (Letter, 612x792)
  * a content stream drawing the text "No Info"
  * a Type1 Helvetica font resource
  * a trailer with no /Info key -- the /Info dictionary is optional per
    the PDF spec, so a conforming reader must not error on its absence
...
def build():
    ...
    stream = b"BT /F1 24 Tf 72 720 Td (No Info) Tj ET\n"
...
if __name__ == "__main__":
    dest = sys.argv[1] if len(sys.argv) > 1 else "no_info.pdf"
    data = build()
